import { blockTag } from "./tag-helpers/block-tag";
import { textNode } from "./tag-helpers/text-node";
import { a } from "./tags/a";
import { abbr, acronym, dfn } from "./tags/abbr";
import { address } from "./tags/address";
import { article, aside, blink, div, figcaption, footer, form, header, hgroup, label, main, nav, p, picture, section, span } from "./tags/base-tags";
import { blockquote } from "./tags/blockquote";
import { br } from "./tags/br";
import { center } from "./tags/center";
import { code, pre } from "./tags/code";
import { dd, dl, dt } from "./tags/definitions";
import { details } from "./tags/details";
import { body, html } from "./tags/document";
import { fieldset } from "./tags/fieldset";
import { figure } from "./tags/figure";
import { font } from "./tags/font";
import { h1, h2, h3, h4, h5, h6 } from "./tags/headers";
import { hr } from "./tags/hr";
import { img } from "./tags/img";
import { button, input, output } from "./tags/inputs";
import { li, ol, ul } from "./tags/list";
import { progress } from "./tags/progress";
import { caption, table } from "./tags/table";
import {
	b,
	big,
	bold,
	cite,
	data,
	del,
	em,
	i as index,
	ins,
	italic,
	kbd,
	mark,
	q,
	s,
	samp,
	small,
	strike,
	strikethrough,
	strong,
	sub,
	sup,
	time,
	tt,
	u,
	underline,
	variableTag,
	wbr,
} from "./tags/text-styles";
import {
	applet,
	area,
	audio,
	base,
	basefont,
	bgsound,
	embed,
	head,
	keygen,
	link,
	map,
	meta,
	noscript,
	optgroup,
	option,
	param as parameter,
	script,
	select,
	source,
	style,
	summary,
	template,
	track,
	video,
} from "./tags/void";

const __text = textNode;

//
export default {
	"#text": __text,
	a,
	abbr,
	acronym,
	address,
	applet,
	area,
	article,
	aside,
	audio,
	b,
	base,
	bgsound,
	big,
	blockquote,
	blockTag,
	body,
	bold,
	br,
	button,
	caption,
	center,
	cite,
	code,
	data,
	dd,
	del,
	details,
	dfn,
	div,
	dl,
	dt,
	em,
	embed,
	fieldset,
	figcaption,
	figure,
	font,
	footer,
	form,
	h1,
	h2,
	h3,
	h4,
	h5,
	h6,
	head,
	header,
	hgroup,
	hr,
	html,
	i: index,
	img,
	input,
	ins,
	italic,
	kbd,
	keygen,
	label,
	li,
	link,
	main,
	map,
	mark,
	meta,
	nav,
	noscript,
	ol,
	optgroup,
	option,
	output,
	p,
	param: parameter,
	picture,
	pre,
	progress,
	q,
	s,
	samp,
	script,
	section,
	small,
	select,
	source,
	span,
	blink,
	strike,
	strikethrough,
	strong,
	style,
	sub,
	summary,
	sup,
	table,
	template,
	time,
	track,
	tt,
	u,
	ul,
	underline,
	var: variableTag,
	video,
	wbr,
	basefont,
};
