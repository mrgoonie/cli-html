...
import { getAttribute, indentify } from "../utils";
import { getListItemNumber, getListSymbol, getListType } from "../utils/list";
...
export const li = (tag, context) => {
	if (context.orderedList) {
		return blockTag((value) => `${blue(`${getListItemNumber(context.liItemNumber, context.listType)}.`)} ${indentify("   ", true)(value)}`)(tag, {
			...context,
			lineWidth: context.lineWidth - 3,
		});
	}

	return blockTag((value) => `${indentify("  ")(value)}`.replace(/ {2}/, `${red(getListSymbol(context.listType))} `))(tag, {
		...context,
